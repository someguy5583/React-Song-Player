...
function App() {
  ...
  if (!currentSong) return <div className="text-neutral-900">Loading...</div>;
  ...
  const [liricleReady, setLiricleReady] = useState(false);
  ...
  useEffect(() => {
    ...
    const handleLoad = (data) => {
      console.log("Liricle loaded:", data.lines.length, "lines");
      setLiricleReady(true);
    };

    const handleSync = (line) => {
      if (line && line.text) {
  ...
  useEffect(() => {
    if (lyrics) {
      setLiricleReady(false); // Reset ready state when song changes
      console.log("Loading lyrics into Liricle...");
      liricleRef.current.load({ text: lyrics });
    }
  }, [lyrics]);

  useEffect(() => {
    if (liricleReady && lyrics) {
      liricleRef.current.sync(value);
    }
  }, [value, liricleReady, lyrics]);
    ...
    <>
      <div className="h-screen bg-gradient-to-tl from-blue-700 to-blue-400 flex justify-center items-center p-0 m-0">
        <div className="bg-neutral-50 h-183 w-100 rounded-4xl p-0 overflow-hidden" id="app">
          ...
          <div className="audioControl w-full px-10 mt-5">
            <div className="timeControl flex-col justify-center items-center -mt-6">
              ...
              <div className="controlsContainer w-full flex items-center justify-center gap-9 p-10 mt-3">
                ...
                >
                  ...
                  <svg
                    ...
                    strokeWidth="1.25"
                    stroke="currentColor"
                    className={`w-8 h-8 translate-x-0.5 absolute transition-all duration-300 ${isPlaying ? "opacity-0 scale-75" : "opacity-100 scale-100"}`}
                  >
                  ...
                  <svg
                    ...
                    stroke="currentColor"
                    strokeLinecap="round"
                    className={`w-9 h-9 absolute transition-all duration-300 ${isPlaying ? "opacity-100 scale-100" : "opacity-0 scale-75"}`}
                  >
            ...
            <div
              className="bottomControl w-full h-13.5 mt-0.5 hover:-mt-8.5 rounded-t-xl flex justify-evenly items-center z-50"
              style={{
